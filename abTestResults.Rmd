...
observeEvent(c(input$metric, input$cva, input$cvb, input$traffa, input$traffb, input$confinterval, input$tails, input$nonf, input$bonf), {
  
  # Don't execute unless traffic inputs are greater than conversion inputs
  req(input$metric == "Binomial")
...
# Main event observer to listen for input changes and to calculate main results
observeEvent(c(input$metric, input$mean_a, input$mean_b, input$traffa_c, input$traffb_c, input$sd_a, input$sd_b, input$confinterval, input$tails, input$nonf, input$bonf), {
  
  # Don't execute unless traffic inputs are greater than conversion inputs
  req(input$metric == "Continuous")
  req(input$mean_a > 0)
  req(input$mean_b > 0)
  req(input$sd_a > 0)
  req(input$sd_b > 0)
...
inputPanel(
    ...
    sliderTextInput("metric", label = "Select your metric type", grid = FALSE, force_edges = TRUE,
    choices = c("Binomial", "Continuous")),
  div(id = "metric_instr",
      "Conversion rates and proportions are binomial metrics. Revenue, time-on-site and such are continous metrics. For continuous, you'll need to enter a standard deviation calculated separately.")
)
...
observeEvent(input$metric,{
  if (input$metric == "Binomial") {
    ...
    output$results_inputs <- renderUI({
      inputPanel(
        h4("Control Variation"),
        numericInput("mean_a", label = "Control Mean", value = 2.57, min = 0),
        numericInput("sd_a", label = "Control Standard Deviation", value = 19, min = 0),
        numericInput("traffa_c", label = "Control Traffic", value = 10000, min = 0),
      
        h4("Test Variation"),
        numericInput("mean_b", label = "Test Mean", value = 2.98, min = 0),
        numericInput("sd_b", label = "Test Standard Deviation", value = 24, min = 0),
...
renderUI({
  ...
  if (input$metric == "Binomial") {
    labs_results <- c(
      paste0(round(rctRslts$cvra,1),"%"),
      paste0(round(rctRslts$cvrb,1),"%")
    )
  } else {
    labs_results <- c(
...
rmarkdown::render_delayed({
  renderPlot({
    ...
    ax_lab_x <- if_else(input$metric == "Binomial", "True difference in conversion rates", "True difference in means")
...
render_gt({
  ...
  gt(df) %>%
    ...
    fmt_number(columns=vars(Lower,Observed,Upper), rows=metric_vars$num_r, decimals = 4) %>%
